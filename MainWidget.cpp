    ...
    ui(new Ui::Widget)
{
    ...
    ui->btnCurMusic->setCheckable(true);  // 使按钮可切换
    ui->btnLocalMusic->setCheckable(true);  // 使按钮可切换
    ui->btnFavorMusic->setCheckable(true);  // 使按钮可切换

    // 创建按钮组，确保只有一个按钮被选中
    QButtonGroup *buttonGroup = new QButtonGroup(this);
    buttonGroup->addButton(ui->btnCurMusic);
    buttonGroup->addButton(ui->btnFavorMusic);
    buttonGroup->addButton(ui->btnLocalMusic);
    buttonGroup->setExclusive(true);  // 确保只有一个按钮可以被选中

    // 初始状态下没有按钮被选中
    ui->btnCurMusic->setChecked(false);   // 初始状态为未按下
    ui->btnLocalMusic->setChecked(false);   // 初始状态为未按下
    ui->btnFavorMusic->setChecked(false);   // 初始状态为未按下
...
{
    QSettings mysettings("./MyMusicPlayer.ini",QSettings::IniFormat);
...
void MainWidget::updateDuration(qint64 duration)
{
    ...
    if(!(static_cast<int>(duration) > 0)) {
        ...
        mySystemTray->setToolTip(u8"MyMusicPlayer · By 栗悟饭与龟波功");
...
{
    QMessageBox::about(this,u8"关于","MyMusicPlayer | 一款精致小巧的本地音乐播放器\n"
                                   "作者：栗悟饭与龟波功\n\n"
...
void MainWidget::on_positionSlider_sliderReleased()
{
